...
namespace trees{
	...
	struct crtp_binary_node : binary_node<T, crtp_binary_node>{
		crtp_binary_node() :binary_node() {}
		explicit crtp_binary_node(T&& key) :binary_node(std::forward<T>(key)) {}
		...
		crtp_binary_node(crtp_binary_node&& other) :binary_node(std::move(other))
		{}
